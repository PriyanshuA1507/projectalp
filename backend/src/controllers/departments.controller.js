...
const enterData = asyncHandler(async (req, res) => {
    ...
    // 1. Ensure Department ID is unique
    if (data.department_id) {
        const deptExists = await Department.exists({ department_id: data.department_id });
        if (deptExists) throw new ApiError(400, "A Department with this ID already exists.");
    }
    // -------------------------------------------
